...
struct ignorecase_traits : public std::char_traits<char> {
	...
	// return wheter c1 is less than c2
...
// define an output operator becase the traits type is different than that for std:: ostream 
inline std::ostream& operator << (std::ostream& stream, const icstring & s){
	return stream << std::string(s.data(), s.length());
}
